"""
cybercop_pipeline_AdotX_v0_2.py — CoT 분류 + Grounding DINO 증거 탐지 통합 파이프라인.

기존 cybercop_pipeline_AdotX.py 대비 주요 변경점은 CHANGELOG.md, v0.1 대비 이번 버전 변경점은
CHANGELOG_0.2.md 참고.
...
def process_image_sequence(dir_path: Path, label, processor, model, gdino: GroundingDetector,
                            rag: CrimeRAG, ocr: PaddleOCR, args,
                            obj_mapper: ObjectMapper | None = None) -> str | None:
...
# ──────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="cybercop pipeline AdotX_v0_2 — CoT + Grounding DINO")
    ...
    parser.add_argument("--out_dir",              default="./output_AdotX_v0.2")
    ...
    if args.url:
        ...
        if not seq_path.is_dir():
            ...
            sys.exit(1)
        pred = process_image_sequence(seq_path, "manual", processor, model, gdino, rag, ocr, args, obj_mapper)
        if pred:
            preds.append(("unknown", pred))
    elif args.csv:
