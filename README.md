...
| 단계 | 이름 | 실행 조건 | 설명 |
|---|---|---|---|
| 1 | 초기 사기/정상 CoT 분류 | 모든 영상 | 대표 프레임 최대 4장을 VLM에 입력. ① 화면 내용을 판단 없이 서술 → ② 서술을 8개 사기 패턴 체크리스트에 대입해 사기/정상 판단 |
| 2 | Grounding DINO 객체 추출 & 에스컬레이션 | 모든 영상 | 계좌 화면, 카카오톡/텔레그램 화면, 신분증, QR코드 등 사기 증거 어휘 36종을 zero-shot으로 탐지. 1단계가 "정상"으로 판단했어도 `evidence_score ≥ 0.15`이면 "검토필요"로 재분류 |
| 3 | OCR 클러스터링 + 객체 리스트 | 1단계 결과가 사기/검토필요일 때만 | PaddleOCR로 화면 텍스트 추출 → 여러 프레임에서 반복 등장하는 텍스트를 위치+문자열 유사도로 클러스터링, 다수결로 오탈자 정리. 객체는 DINO 탐지 결과와 VLM이 자유 서술한 객체 목록을 병합 |
| 4 | RAG 검색 (범죄 유형 매칭) | 1단계 결과가 사기/검토필요일 때만 | 정리된 OCR 텍스트 + 객체 목록을 하나의 쿼리로 결합, 범죄 유형 문서 DB(15개 대분류)에서 임베딩 유사도로 최근접 유형 검색 |
